...
static inline uint64_t resolve_libc_function(pid_t pid, const char *func) {
  map_range_t range = {0};
  ...
  while (read_bytes < size) {
    if ((ret = fread(buf+read_bytes, 1, size-read_bytes, fdi)) < 0) {
      fatal("error fetching process memory: %llx - %llx (%ld/%ld)", range.low,
            range.high, read_bytes, size);
    }
    if (ret == 0) {
      break;
    }
  ...
  if (read_bytes < size) {
    printf("could not read entire process memory...\n");
  }

  fclose(fdi);

  Elf64_Ehdr *ehdr = (Elf64_Ehdr *)buf;
  printf("ELF bits: %d\n", ehdr->e_ident[EI_CLASS] == 2 ? 64 : 32);
...
int main(int argc, char **argv) {
  if (argc != 3) {
    printf("usage: %s <pid> <function>\n", argv[0]);
  } else {
    pid_t pid = atoi(argv[1]);
    char *name = argv[2];
    printf("searching loaded version of libc in %d for %s...\n", pid, argv[1]);
    uint64_t addr = resolve_libc_function(pid, name);
    if (addr != 0) {
      printf("found %s @ %#lx\n", name, addr);
    } else {
      puts("could not find function");
    }
  }
  return 0;
}
